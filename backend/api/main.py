...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"⚠️ Warning: Could not load models. Ensure train_all.py was run. Error: {e}")
    yield
...
def process_transactions(df: pd.DataFrame, borrower_id: str):
    ...
    # Run ML Inference
    anomaly_score = ml_models["m2"].decision_function(features)[0] # Raw anomaly score
    ...
    if flags > 0 or is_anomaly == -1:
        if flags >= 2 or anomaly_score < -0.1:
    ...
    raw_gci = 300 + (stability_score * 0.30) + (liquidity_score * 0.25) + (repayment_score * 0.20) + (discipline_score * 0.15) - (cvd_variance * 100 * 0.10)
    final_gci = int((raw_gci) * penalty)
...
def get_borrowers(db: Session = Depends(get_db)):
    records = db.query(BorrowerDB.borrower_id).all()
    return {"borrower_ids": [r[0] for r in records]}
